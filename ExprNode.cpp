...
ArrayInitNode::ArrayInitNode(Token token): ExprNode{token} {}

void ArrayInitNode::print() {
    for ( auto i : expr_nodes){
        i->print();
        std::cout << ",";
    }
}
...
// this must return an array type descriptor
std::shared_ptr<TypeDescriptor> ArrayInitNode::evaluate(SymTab &symTab, Functions &funcTab){
    std::shared_ptr<TypeDescriptor> expr_node_td;
    std::shared_ptr<StringArrayTypeDescriptor> str_array_td;
    std::shared_ptr<NumberArrayTypeDescriptor> nbr_array_td;

    if(expr_nodes.size() > 0){ // make sure it is non-empty

        expr_node_td = expr_nodes[0]->evaluate(symTab, funcTab); // eval 1st item to determine the data type

        if(expr_node_td->type() == TypeDescriptor::INTEGER){
            std::vector<int> new_int_vector;
            nbr_array_td = std::make_shared<NumberArrayTypeDescriptor>(TypeDescriptor::NUMBER_ARRAY); // make it a number array

            std::shared_ptr<NumberTypeDescriptor> nbr_td_ptr = std::dynamic_pointer_cast<NumberTypeDescriptor>(expr_node_td);
            new_int_vector.push_back(nbr_td_ptr->value.intValue);

            for(int i = 1; i < expr_nodes.size(); i++){
                expr_node_td = expr_nodes[i]->evaluate(symTab, funcTab);
                if(expr_node_td->type() == TypeDescriptor::INTEGER){
                    nbr_td_ptr = std::dynamic_pointer_cast<NumberTypeDescriptor>(expr_node_td);
                    new_int_vector.push_back(nbr_td_ptr->value.intValue);
                }
                else{
                    std::cout << "ERROR, INTEGER DATATYPE EXPECTED\n";
                    exit(1);
                }
            }
            nbr_array_td->setValue(new_int_vector);
            return nbr_array_td;
        }
        else if(expr_node_td->type() == TypeDescriptor::STRING){
            std::vector<std::string> new_string_vect;
            str_array_td = std::make_shared<StringArrayTypeDescriptor>(TypeDescriptor::STRING_ARRAY); // make it a string array

            std::shared_ptr<StringTypeDescriptor> str_td_ptr = std::dynamic_pointer_cast<StringTypeDescriptor>(expr_node_td);
            new_string_vect.push_back(str_td_ptr->stringValue);

            for(int i = 1; i < expr_nodes.size(); i++){
                expr_node_td = expr_nodes[i]->evaluate(symTab, funcTab);
                if(expr_node_td->type() == TypeDescriptor::STRING){
                    str_td_ptr = std::dynamic_pointer_cast<StringTypeDescriptor>(expr_node_td);
                    new_string_vect.push_back(str_td_ptr->stringValue);
                }
                else{
                    std::cout << "ERROR, STRING DATATYPE EXPECTED\n";
                    exit(1);
                }
            }
            str_array_td->setValue(new_string_vect);
            return str_array_td;
        }
        else{
            std::cout << "EEROR WITHIN ExprNode::ArrayInitNode::evaluate()\n";
            exit(1);
        }
    }
}
