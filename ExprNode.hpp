...
// to be used to array init.
class ArrayInitNode: public ExprNode {
public:
    ArrayInitNode(Token token); // datatype?
    virtual void print();
    virtual std::shared_ptr<TypeDescriptor> evaluate(SymTab &symTab, Functions &funcTab);
    void setExprNodes(std::vector<ExprNode*> expr_nd)   {expr_nodes = expr_nd;}
private:
