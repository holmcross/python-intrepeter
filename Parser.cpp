...
Statement* Parser::simple_statement(){
    ...
    if(tok.isPrintKeyword() ){
        ...
        std::cout << "Parser::simple_statement() Array operation incoming\n";
        tokenizer.ungetToken();
        newStatement = array_ops();
    }
...
AssignmentStatement *Parser::assign_statement() {
    ...
    Token tok = tokenizer.getToken();
    if(tok.isOpenSquareBrackets() ){
        tokenizer.ungetToken();
        std::vector<ExprNode*> vectorOfRHS = array_init(); // we need to figure out how to handle this
        ArrayInitNode* array_init_node = new ArrayInitNode(tok);
        array_init_node->setExprNodes(vectorOfRHS);
        std::cout << "returning a new array initialization";
        AssignmentStatement* new_ptr = new AssignmentStatement(nameTok.getName(), array_init_node);
        new_ptr->setArrayInit(true);
        return new_ptr;
    }
    else{
        tokenizer.ungetToken();
        ExprNode *rightHandSideExpr = test();
        AssignmentStatement* new_ptr = new AssignmentStatement(nameTok.getName(), rightHandSideExpr);
        new_ptr->setVariableAssign(true);
        return new_ptr;
    }
...
std::vector<ExprNode*> Parser::array_init(){
    Token tok = tokenizer.getToken();
    if(!tok.isOpenSquareBrackets()){
        die("Parser::array_init()", "Expected an open bracket token, instead received ", tok);
    }
    std::vector<ExprNode*> expr_nodes = testList();

    tok = tokenizer.getToken();
    if(!tok.isClosedSquareBrackets()){
        die("Parser::array_init()", "Expected an close bracket token, instead received ", tok);
    }
    return expr_nodes;
}

Statement* Parser::array_ops(){
    Token nameTok = tokenizer.getToken();
    if(!nameTok.isName()){
        die("Parser::arrary_ops()", "Expected a name token, instead got ", nameTok);
    }

    Token tok = tokenizer.getToken();
    if(!tok.isPeriod()){
        die("Parser::arrary_ops()", "Expected a period token, instead got ", tok);
    }

    Token opName = tokenizer.getToken();
    if(opName.isArrayAppend()){
        tok = tokenizer.getToken();
        if(!tok.isOpenParen()){
            die("Parser::arrary_ops()", "Expected a open paren token, instead got ", tok);
        }

        ExprNode* expr_node = test();

        tok = tokenizer.getToken();
        if(!tok.isCloseParen()){
            die("Parser::arrary_ops()", "Expected a close paren token, instead got ", tok);
        }
        return new ArrayOperationStatement(nameTok.getName(), opName.getName(), expr_node);
    }
    else if(opName.isArrayPop()){
        tok = tokenizer.getToken();
        if(!tok.isOpenParen()){
            die("Parser::arrary_ops()", "Expected a open paren token, instead got ", tok);
        }
        ...
        return new ArrayOperationStatement(nameTok.getName(), opName.getName());
    }
    else{
        std::cout << "Error in Parser::array_ops, operation not supported\n";
        exit(1);
    }
}
