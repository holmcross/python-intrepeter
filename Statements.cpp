...
ArrayOperationStatement::ArrayOperationStatement(): array_name{""}, op_name{""}, expr_node{nullptr} {}
ArrayOperationStatement::ArrayOperationStatement(std::string ary_name, std::string op, ExprNode* expr ): array_name{ary_name}, op_name{op}, expr_node{expr} {} 

void ArrayOperationStatement::evaluate(SymTab &symTab, Functions &funcTab){
    if(op_name == "append"){

        std::shared_ptr<TypeDescriptor> append_value = expr_node->evaluate(symTab, funcTab);
        std::shared_ptr<TypeDescriptor> td_value = symTab.getValueFor(array_name);
        if( (append_value->type() == TypeDescriptor::INTEGER) && (td_value->type() == TypeDescriptor::NUMBER_ARRAY) ){
            std::shared_ptr<NumberTypeDescriptor> num_td_ptr = std::dynamic_pointer_cast<NumberTypeDescriptor>(append_value);
            std::shared_ptr<NumberArrayTypeDescriptor> num_array_td_ptr = std::dynamic_pointer_cast<NumberArrayTypeDescriptor>(td_value);
            std::cout << "Appending value " << num_td_ptr->value.intValue << " onto " << array_name << std::endl;
            num_array_td_ptr->intVector.push_back( num_td_ptr->value.intValue );
        }
        else if(append_value->type() == TypeDescriptor::STRING && td_value->type() == TypeDescriptor::STRING_ARRAY){
            std::shared_ptr<StringTypeDescriptor> str_td_ptr = std::dynamic_pointer_cast<StringTypeDescriptor>(append_value);
            std::shared_ptr<StringArrayTypeDescriptor> str_array_td_ptr = std::dynamic_pointer_cast<StringArrayTypeDescriptor>(td_value);
            std::cout << "Appending value " << str_td_ptr->stringValue << " onto " << array_name << std::endl;
            str_array_td_ptr->stringVector.push_back( str_td_ptr->stringValue );
        }
        else{
            std::cout << "Statements::ArrayOperationStatement::evaluate() ... type mismatch\n";
            exit(1);
        }
    }
    else if(op_name == "pop"){
        std::shared_ptr<TypeDescriptor> td_value = symTab.getValueFor(array_name);
        if(td_value->type() == TypeDescriptor::NUMBER_ARRAY){
            std::shared_ptr<NumberArrayTypeDescriptor> num_array_td_ptr = std::dynamic_pointer_cast<NumberArrayTypeDescriptor>(td_value);
            if(num_array_td_ptr){
                std::cout << "Popping " << num_array_td_ptr->intVector.back() << " from " << array_name << std::endl;
                num_array_td_ptr->intVector.pop_back();
            }
        }
        else if (td_value->type() == TypeDescriptor::STRING_ARRAY){
            std::shared_ptr<StringArrayTypeDescriptor> str_array_td_ptr = std::dynamic_pointer_cast<StringArrayTypeDescriptor>(td_value);
            if(str_array_td_ptr){
                std::cout << "Popping " << str_array_td_ptr->stringVector.back() << " from " << array_name << std::endl;
                str_array_td_ptr->stringVector.pop_back();
            }
        }
        else{
            std::cout << "TD Type not supported\n";
            exit(1);
        }
    }
}
