...
Token Tokenizer::getToken() {
    ...
    //if(inStream.eof()) { // we need to check for end of file any time we would grab a character
    ...
    if( inStream.eof()) {
        ...
        if(inStream.peek() == '('){
            ...
            std::cout << "READ AN ARRAY OP: ";
        }
        else if(inStream.peek() == '['){
            token.isArrayName() = true;
            std::cout << "READ AN ARRAY NAME: ";
        }
        token.setName( word );
    } 
    else {
        std::cout << "Unknown character in input. ->" << int(c) << "<-" << std::endl;
